...
class MCPServer:
    ...
    async def run_sse(self, host: str, port: int):
        """Run the MCP server with SSE endpoints for Lyzr Studio."""
        ...
        async def messages_endpoint(request: Request):
            ...
            if method == "initialize":
                ...
                response = {
                    "jsonrpc": "2.0",
                    "id": req_id,
                    "result": {"tools": [t.dict() if hasattr(t, 'dict') else {"name": t.name, "description": t.description, "inputSchema": t.inputSchema} for t in tools]}
                }
...
def main():
    ...
    parser.add_argument("--stdio", action="store_true", help="Run in stdio mode instead of SSE")
